...
class FoodDashboardViewModel {
    ...
    fileprivate func convertToModel(_ items: [Item]) -> [FoodCellModel] {
        return items.map { FoodCellModel(image: UIImage(named: $0.image),
                                         imageName: $0.image,
                                         name: $0.name,
            ...
            case .success(let response) :
                ...
                for food in response.foods {
                    if food.categoryCode == filter.rawValue {
                        ...
                        let pizzaItems = self.convertToModel(items)
